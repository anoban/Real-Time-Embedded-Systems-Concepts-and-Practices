...
int main(void) {
    ...
    if ((childprocess_id = fork()) > 0) { // if the current process is the parent
        ...
        fputws(
            L"Inside parent process!", stdout
        ); // stdouts from parallel processes can interleave since these aren't guarded (here) by the semaphore.
        for (size_t i = 0; i < ITERMAX; ++i) {
            ...
            wprintf(
                L"gsum is %llu: Message from parent\n", sumg
            ); // putting this inside semaphore locked segment to avoid stdout being affected by process interleaving
        ...
        // parent proc was the one that created the semaphores, so it has to unlink those semaphores!
        // IT ALONE HAS TO UNLINK THEM WHILE CLOSING NEEDS TO HAPPEN IN BOTH PROCESSES
        ...
        for (size_t i = 0; i < ITERMAX; ++i) { // close and open semaphores ITERMAX times iteratively
            ...
            wprintf(
                L"gsum is %llu: Message from child\n", sumg
            ); // putting this inside semaphore locked segment to avoid stdout being affected by process interleaving
